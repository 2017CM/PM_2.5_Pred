...
option = st.sidebar.radio(
    '实验步骤',
    ('小组信息','数据集概览','描述性分析','参数优化', '模型评估')
)

# 根据选择的目录项在主面板中显示相应的内容
if option == '小组信息':
    st.title("Group 9 小组信息")
    group=pd.DataFrame({
        '姓名':['陈喆','孙浩楠','喻培原','于涵','张艺琳'],
        '学号':['2330233009','2330233067','2330233105','2330232171','2330232184']
        })
    # 设置表格样式
    # 将DataFrame转换为HTML字符串，不添加索引，将第一行加粗
    group_html = group.to_html(index=False, header=True, bold_rows=True)
    # 使用st.markdown来显示表格
